...
elmin = 5               # minimum elevation for float solution (degrees)
cnr_min = [28, 20]   # min signal strength [freq1, freq2] (dB-Hz)
excsats = []             # excluded sats

maxinno = 5              # outlier threshold for phase (m)
maxcode = 10             # outlier threshold for code (m)
maxage = 30              # mag age of differential
maxout = 1               # maximum outage [epoch]
thresdop = 10             # cycle slip detection by doppler method
thresslip = 0.10         # cycle slip detection by geom-free LC
interp_base = False      # interpolate base observations

# ------------  Kalman Filter Statistics ------------------------
eratio = [200, 25]    # ratio between psuedorange noise and carrier phase noise for L1, L5
efact = {uGNSS.GPS: 1.0, uGNSS.GLO: 1.5, uGNSS.GAL: 1.0} # relative weighting of each constellation
#err = [0, 0.00, 0.003, 0.003, 0.25, 5e-12]  # err sigmas [-, base, el, snr, rcvstd, bl, satclk]
err = [0, 0.005, 0.000, 0.000, 0.005, 0, 0]  # err sigmas [-, base, el, bl, snr, rcvstd, satclk]
snrmax = 45              # max signal strength for variance calc (dB-Hz)
accelh = 0.5               # horiz accel noise sigma (m/sec2)
accelv = 0.1               # vert accel noise sigma (m/sec2)
prnbias = 0.001           # Carrier phase bias sigma ( cycles)
...
thresar1 = 1          # max pos variation for AR (and accel update in kalman filter)
...
#rr_f = [-2524054.942058, -4640745.673847,  3562362.024389,
#        0.161032,        0.122775,       -0.166445]
